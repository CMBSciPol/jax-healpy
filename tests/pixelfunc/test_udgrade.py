...
def test_udgrade_basic_compatibility(nside_in, nside_out, order_in):
    ...
    # Convert to specified ordering if needed
    if order_in == 'NESTED':
        # healpy uses nest=True for NESTED ordering
        nest_in = True
        nest_out = True  # Keep same ordering by default
    else:
        nest_in = False
        nest_out = False
    ...
    # Verify exact match
    assert_array_equal(
        jax_result, hp_result, 
        f"udgrade mismatch: nside {nside_in}->{nside_out}, order {order_in}"
    )
...
def test_udgrade_round_trip_consistency(nside):
    ...
    assert_allclose(
        round_trip, original_map, rtol=1e-10, atol=1e-10,
        err_msg=f"Round-trip failed for nside={nside}"
    )
...
def test_udgrade_interface_validation():
    ...
    result = jhp.udgrade(test_map, 8, pess=True, order_in='RING', order_out='NESTED', 
                       power=2.0, dtype=np.float32)
...
def test_udgrade_power_parameter(power, nside_in, nside_out):
    ...
    assert_allclose(jax_result, hp_result, rtol=1e-6, atol=1e-6,
...
def test_udgrade_cross_ordering(order_in, order_out, nside_in, nside_out):
    ...
    assert_array_equal(jax_result, hp_result, 
                      f"Cross-ordering {order_in}→{order_out} should match healpy")
...
def test_udgrade_pessimistic_detailed(pess, nside_degradation):
    ...
    assert_array_equal(jax_result, hp_result, 
                      f"Pessimistic={pess} detailed UNSEEN handling should match healpy")
...
def test_udgrade_advanced_combinations(map_type, order_combo):
    ...
    assert_array_equal(jax_result, hp_result, 
                      f"Advanced combo {map_type} maps {order_in}→{order_out} with power should match healpy")
...
def test_udgrade_jit(nside):
    ...
    assert_array_equal(result_direct, result_jit, 
                      f'nside {nside}: JIT and direct results must match')
    ...
    assert_array_equal(result_multi_direct, result_multi_jit,
                      f'nside {nside}: JIT multiple maps must match direct call')
    ...
    if nside >= 8:  # Need sufficient resolution for degrade
        ...
        assert_allclose(result_advanced_direct, result_advanced_jit, rtol=1e-6,
    ...
    assert_array_equal(result_cached_1, result_cached_2,
                      f'nside {nside}: JIT cached calls must be consistent')
...
def test_udgrade_gradient(nside):
    ...
    assert_allclose(gradients, expected_grad, rtol=1e-6,
...
def test_udgrade_gradient_degrade():
    ...
    assert_allclose(gradients, expected_grad, rtol=1e-6,
                   err_msg='Degrade gradients should equal 1/averaging_factor')
...
def test_udgrade_gradient_with_power():
    ...
    assert_allclose(gradients, expected_grad, rtol=1e-5,
                   err_msg='Power-scaled gradients should include power factor')
...
def test_udgrade_jit_advanced_parameters(order_combo, pess):
    ...
    result1_direct = jhp.udgrade(test_map, nside_out, pess=pess, order_in=order_in, 
                                order_out=order_out, power=2.0, dtype=np.float32)
    result1_jit = jit_udgrade(test_map, nside_out, pess=pess, order_in=order_in, 
                             order_out=order_out, power=2.0, dtype=np.float32)
    
    assert_allclose(result1_direct, result1_jit, rtol=1e-6,
                   err_msg=f'JIT advanced params {order_combo} pess={pess} must match direct call')
    
    # Call 2: Test JIT caching with same parameters
    result2_jit = jit_udgrade(test_map, nside_out, pess=pess, order_in=order_in, 
                             order_out=order_out, power=2.0, dtype=np.float32)
    
    assert_allclose(result1_jit, result2_jit, rtol=1e-12,
                   err_msg=f'JIT cached calls {order_combo} pess={pess} must be identical')
    
    # Call 3: Test with different parameters to ensure no caching conflicts
    result3_jit = jit_udgrade(test_map, nside_out, pess=not pess, order_in=order_in, 
                             order_out=order_out, power=1.0, dtype=np.float64)
    result3_direct = jhp.udgrade(test_map, nside_out, pess=not pess, order_in=order_in, 
                                order_out=order_out, power=1.0, dtype=np.float64)
    
    assert_array_equal(result3_direct, result3_jit,
                      f'JIT different params {order_combo} must work correctly')
...
def test_udgrade_jit_multiple_maps():
    ...
    for case_name, test_data in test_cases:
        ...
        assert_array_equal(result_direct, result_jit,
                          f'JIT {case_name} maps must match direct call')
        ...
        assert_array_equal(result_f32_direct, result_f32_jit,
                          f'JIT {case_name} maps with dtype must match direct call')
