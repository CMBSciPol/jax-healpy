...
def get_interp_weights(
    ...
    pixels, weights = _interp_pixels_and_weights_ring(nside, theta, phi)
    if nest:
...
def _interp_pixels_and_weights_ring(nside: int, theta: Array, phi: Array) -> tuple[Array, Array]:
    ...
    theta_up = _ring2theta(nside, iring_up)
    jleft_up, jright_up, w_up = _left_right_weights_ring(nside, iring_up, phi)
    #   if (ir1>0)
    #     {
    ...
    theta_lo = _ring2theta(nside, iring_lo)
    jleft_lo, jright_lo, w_lo = _left_right_weights_ring(nside, iring_lo, phi)
    #   if (ir2<(4*nside_))
    #     {
...
def _left_right_weights_ring(nside: int, iring: ArrayLike, phi: ArrayLike) -> tuple[Array, Array, Array]:
...
def _ring2theta(nside: int, i_ring: ArrayLike) -> Array:
    z, abs_one_minus_z = _ring2z(nside, i_ring)
    theta = jnp.where(
        jnp.abs(z) > 0.99,
        jnp.arctan2(jnp.sqrt(abs_one_minus_z * (2 - abs_one_minus_z)), z),
        jnp.arccos(z),
...
def _ring2z(nside: int, i_ring: ArrayLike) -> tuple[Array, Array]:
    abs_one_minus_z = _pix2z_polar_caps_ring(nside, i_ring)
    z = jnp.where(
        i_ring < nside,  # north polar cap
        1 - abs_one_minus_z,
        jnp.where(
            i_ring > 3 * nside,  # south polar cap
            abs_one_minus_z - 1,
            _pix2z_equatorial_region_ring(nside, i_ring),
        ),
    )
    return z, abs_one_minus_z
